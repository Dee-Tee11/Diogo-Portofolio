# Diogo Teixeira
### AI Engineer & Full Stack Developer

Computer Engineer specializing in AI and Full Stack development. Experienced in delivering full-stack automation projects and integrating cutting-edge Generative AI into enterprise solutions. Dedicated to engineering excellence and continuous innovation.

---

## 🛠️ Tech Stack
- **Languages:** Python, TypeScript, Java, C#, C++, SQL
- **Front-End:** React, Next.js, Three.js, Framer Motion, Tailwind CSS
- **Back-End:** FastAPI, Node.js, .NET, MySQL, Vector Databases
- **AI/ML:** Reinforcement Learning, RAG, Generative AI, Machine Learning, Jupyter
- **DevOps:** Azure DevOps, Git, CI/CD Pipelines, Cloud Storage, Edge Functions

---

## 💼 Professional Experience

### **Soluis** | AI, Automation & Full Stack Engineer
*Jan 2025 - Present*
- Participation in various Full Stack automation projects.
- Integration of Artificial Intelligence solutions into enterprise systems.
- Worked with scalable serverless solutions using Cloud Storage and Edge Functions.
- **Stack:** React, TypeScript, Python, Three.js, MySQL, Azure DevOps, C#

### **Samsys** | Curricular Internship - Full Stack Developer
*Sep 2024 - Dec 2024*
- Backend and Frontend development using React, TypeScript & .NET.
- Created a custom plugin implemented in the company's main application.
- Worked with Agile Methodologies (Scrum), Azure DevOps, and Git.

### **Fontys University (Netherlands)** | International Project
*Apr 2024*
- Intensive project: "Transformation from linear to circular economy".
- Applied sustainability and profit strategies for an electric bike company.
- **Technologies:** Java

### **VisitPlann** | Summer Internship - Back-End Developer
*Jul 2022 - Aug 2022*
- Developed a mobile application for monument recognition via location and IA.
- **Technologies:** Android Studio, Java

---

## 🚀 Featured Projects

### **Atlas AI**
AI Secretary with advanced conversational capabilities. Fine-tuned language model using Reinforcement Learning for optimal performance.
- Applied Reinforcement Learning techniques for model fine-tuning.
- Developed and trained on Google Colab and Kaggle platforms.
- **Stack:** Python, Reinforcement Learning, Jupyter, Google Colab, Kaggle

### **Movie Night AI**
Intelligent movie recommendation application. Advanced RAG system with vector embeddings for personalized suggestions.
- Architected scalable serverless solutions using Cloud Storage and Edge Functions.
- Designed and managed CI/CD pipelines for automated DevOps workflows.
- **Stack:** React, TypeScript, Python, RAG, Gen AI, ML

### **3D Interactive Portfolio**
An immersive experience built with React Three Fiber to showcase skills and projects.
- Implemented a responsive 3D scene with interactive elements.
- Optimized performance and animations for smooth user experience.
- **Stack:** React, Three.js, Framer Motion, Vite

---

## 🧠 Soft Skills
- Agile Methodology & Scrum
- Critical Thinking & Problem Solving
- Teamwork & Autonomy
- Fast Learning
- Effective Communication
- Adaptability & Flexibility
- Time Management

---

## 🌍 Languages
- **Portuguese:** Native
- **English:** Fluent (C1)

---
---

# Versão Portuguesa 🇵T

## Diogo Teixeira
### Engenheiro de IA & Desenvolvedor Full Stack

Engenheiro Informático especializado em IA e desenvolvimento Full Stack. Experiente na entrega de projetos de automação full-stack e na integração de IA Generativa de vanguarda em soluções empresariais. Dedicado à excelência na engenharia e à inovação contínua.

### 💼 Experiência Profissional

- **Soluis** | Engenheiro de IA, Automação & Full Stack (2025 - Presente)
- **Samsys** | Estágio Curricular (2024)
- **Projeto Internacional (Países Baixos)** | Fontys University (2024)
- **VisitPlann** | Estágio de Verão (2022)

### 🚀 Projetos em Destaque

- **Atlas AI**: Secretário IA ajustado com Reinforcement Learning.
- **Movie Night AI**: Sistema de recomendação inteligente com RAG e embeddings.
- **Portfólio 3D**: Experiência imersiva com React Three Fiber.

---

## 📬 Contact Data
- **Email:** diogoluisteixeira@gmail.com
- **Phone:** +351 931 069 434
- **LinkedIn:** [Diogo Teixeira](https://www.linkedin.com/in/diogo-teixeira-9b108423b/)
